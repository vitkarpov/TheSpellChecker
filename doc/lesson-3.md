...
Обратите внимание, что исключение всегда должно быть выброшено по значению (не создавайте новый объект через `new`) и быть пойманным по константной ссылке. Это позовляет избежать разного рода проблем с памятью, а также гарантирует, что виртуальные функции при наследовании классов будут работать правильно.

Исключения должны быть пойманные в определенном порядке: сперва отлавливаются наиболее специфичные, т.е. те, у которых больше всего родительских классов. Если этого не сделать, то они пойманы не будут вовсе.

Несмотря на то, что вы явно не определяли функцию `what` вашего класса исключения, она есть — виртуальная функция, которая унаследована от родительского класса `std::exception`. Она возвращает `const char *` ссылку на сообщение об ошибке, которое содержит функция, если настоящий тип исключения поддерживает сообщение об ошибке.

Теперь, если файл словаря не может быть прочитан, пользователь увидит понятно сообщение:
...
Итак, мы разобрались с обработкой ошибок при загрузке словаря, самое время подумать о данных для словаря. Нужен более или менее полный список слов, потому что одна панграмма вряд ли впечатлит вашего босса :)

Немного погуглив, я нашел [архив со списком английских слов](http://sourceforge.net/projects/wordlist/). Следует скачать архив и посмотреть, что в нем. Я не буду говорить подробно о том, что написано в README архива, но если коротко: требуется запустить приложенный скрипт, написанный на perl, который соберет список файлов в нужном для нас виде. В итоге, получился 3,5 мб словарь.

Меня беспокоит скорость работы `std::set` — то, что используется в основе заполнения словаря данными (в скорости поиска я уверен). На 3,5 мб списке заполнения словаря занимает 1,6 секунд на моем старом ноутбуке. Кажется, что это это на нижней границе допустимого, но все же допустимо. Тем не менее, мы еще вернемся к этому, когда будет говорить про производительность.

### Заключение

В итоге, мы обсудили:

* как читатать читать файлы — не используйте `eof()` в условии выхода из цикла;
* всегда нужно проверять результат чтения/записи — могут быть ошибки;
* лучше всего с ошибками работать с помощью исключений;
* реализация словаря удовлетворительная в смысле производительности.
