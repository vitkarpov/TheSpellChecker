...
Нельзя дважды объявлять один и тот же класс или функцию в одном файле. Можно подумать, что никому не придет в голову писать тот же самый `#include` дважды, однако это часто происходит случайно. Чтобы избежать этого нужно использовать возможности препроцессора.
...
Скомпилируйте еще раз — ошибки нет.

Директива `#ifndef` работает следующим образом: в первым раз макрос `INC_DICTIONARY_H` не определ, так что код, который следует далее будет разобран компилятором.

В следующий раз, когда заголовк будет подключен повторно, макрос уже будет определен, а значит компилятор пропустит этот кусок кода.

Хорошей практикой является использование таких директив для каждого заголовка, оборачивая всю декларацию в `#ifndef` и `#endif`.

Есть множество соглашений по именованию макросов, однако стоит помнить, что нельзя использовать двойные подчеркивания, название не должно начинаться с подчеркивания (`__DICTIONARY__`, `_DICTIONARY_H_`) — такие имена зарезервированы компилятором для собственных нужд.

### Назад к Словарю

Самое время приступить к реализации словаря: нужен способ быстро искать слова в списке. Стандартная библиотека располагает широким наобором возможностей для этого. Думаю, что первое на что падает выбор: `std::set`.

В `C++` `std::set` — это коллекция уникальных объектов, которые представляют собой сбалансированное бинарное дерево, поиск по которому очень быстрый.

Разумеется возникает вопрос: откуда я могу узнать о возможностях стандартной библиотеки и когда именно что нужно использовать? Понимае будет приходить с опытом, так же можно прочитать [«The C++ Standard Library» Nicolai Josuttis](http://www.josuttis.com/libbook/).

Итак, что же такое `set`. В действительности это шаблон — требуется указать какого типа данные вы собираетесь хранить. В данном случае, это строки, так что модифицируем заголовочный файл:
...
Теперь, если скомпилировать программу, получим:
...
Теперь разберемся что же в действительности происходит. В конструкторе мы добавляем в коллекцию новый элемент: строку `dog`.

Функция `check` используем метод коллекции `find`, чтобы найти указанную в качестве аргумента строку.

Если указанная строка не была найдена, то метод вернет итератор (можно думать об итераторе как о специальном указателе), который указывает на `end()` коллекции — специальное неиспользуемое значение.

Если же указанная строка найдена, то вернется итератор, указывающий на найденное значение.

Сейчас нужно заполнить коллекцию данными. Вопрос о том где брать список эталонных слов еще не был решен, так что пока просто составим его вручную. Для начала можно просто написать «the quick brown fox jumped over the lazy dog», все в нижнем регистре, каждое слово с новой строки. И сохранить файл с именем `mydict.dat` в директории `data`.

Теперь модифицируем cpp-файл: нужно добавить код, который будет использовать `ifstream` для чтения из файла построчно. В следующей главе это будет рассмотрено подробнее, а пока можно просто написать:
...
Так же нужно изменить вызов конструктора словаря внутри функции `main`, с учетом новой сигнатуры:

```cpp
Dictionary d( "data/mydict.dat" );
```

Теперь функция `main` может быть переписана для чтения слов на проверку из стандартного ввода:
...
* Появление новых требований во время разработки — это нормально
* Классы — один из способов реализации задач, но не единственный
* Ошибки компиляции и линковки могут быть не только синтаксическими
* Стандартная библиотека содержит классы, которые позволяют быстро решать некоторые очевидно сложные задачи

### Что дальше?

[Работа с файлами, обработка ошибок и производительность](lesson-3.md)